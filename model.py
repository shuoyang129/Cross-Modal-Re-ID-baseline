...
class embed_net(nn.Module):
    def __init__(
        self, class_num, no_local="on", pooling_type=0, arch="resnet50",
    ):
        super(embed_net, self).__init__()
        # pooling_type: 0-->avgpooling, 1-->gm_pooling, 2-->similarity,
        #               3-->avgpooling+similarity, 4--> gm_pooling+similarity
        assert (
            pooling_type <= 4
        ), "pooling_type:0-->avgpooling, 1-->gm_pooling, 2-->similarity,3-->avgpooling+similarity, 4--> gm_pooling+similarity"
        ...
        if self.pooling_type == 3 or self.pooling_type == 4:
            pool_dim = 2048 + 162
    ...
    def forward(self, x1, x2, modal=0):
        ...
        # pooling_type: 0-->avgpooling, 1-->gm_pooling, 2-->similarity,
        #               3-->avgpooling+similarity, 4--> gm_pooling+similarity
        if self.pooling_type == 1:
            b, c, h, w = x.shape
            x = x.view(b, c, -1)
            p = 3.0
            x_pool = (torch.mean(x ** p, dim=-1) + 1e-12) ** (1 / p)
        elif self.pooling_type == 0:
